...
The cinematic frontend lives in a sibling repo (`~/panel-v2`): vanilla TypeScript + Vite 8 + Three.js + GSAP, no framework. `build_and_deploy.sh` runs the Vite build, copies the static `dist/` into `app/static/`, validates the Databricks bundle, deploys, and restarts the app.
...
Frontend (in the sibling `panel-v2/` repo):
